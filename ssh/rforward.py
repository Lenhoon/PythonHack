...
"""
# 这个简单的脚本演示如何通过paramiko做远程的端口转发
# 这个脚本连接被请求SSH服务器并且配置远端端口通过一个隧道从远端端口到本机目标可达的一个机器的转发（openssh -R选项）
...
def handler(chan, host, port): # chan是sshserver也就是攻击机的client，host是web
    sock = socket.socket()
    try:
    ...
    while True:  # 数据的发送和接收sock用来和web通信，sock用来和攻击机通信
        r, w, x = select.select([sock, chan], [], []) # 等待完成的一个方法
        if sock in r:
            data = sock.recv(1024)
            if len(data) == 0:
                break
...
def reverse_forward_tunnel(server_port, remote_host, remote_port, transport):
    # transport用来处理和维护加密连接
    # channel像套接字一样在加密传输会话中发送和接收数据
    # 此函数将ssh服务端一个端口的TCP连接转发出去，发出去发到sshserver
    transport.request_port_forward('', server_port)
    while True:
        chan = transport.accept(1000)  # 建立一个新的传输通道接受信息
        if chan is None:
            continue
        thr = threading.Thread(target=handler, args=(
            chan, remote_host, remote_port))
        thr.setDaemon(True) # 设置是否后台运行
        thr.start()
